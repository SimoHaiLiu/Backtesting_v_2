...
class BacktestingEngine(object):
    ...
    def runBacktesting(self):
        ...
        self.output(u'开始回测')
        self.strategy.vtSymbols = ['IF0000']
    ...
    def crossLimitOrder(self):
        ...
        for orderID, order in list(self.workingLimitOrderDict.items()):
            ...
            if buyCross or sellCross:
                ...
                trade.dt = self.dt
                # valuem = [trade.vtSymbol, trade.tradeID, trade.vtTradeID, trade.orderID, trade.vtOrderID,
                #           trade.direction, trade.offset, trade.price, trade.volume, trade.tradeTime, trade.dt]
                # df = pd.DataFrame([valuem],
                #                   columns=['vtSymbol', 'tradeID', 'vtTradeID', 'orderID', 'vtOrderID', 'direction',
                #                            'offset', 'price', 'volume', 'tradeTime', 'dt'])
                # df.to_csv('F:\Work\Code\ tradeBinance.csv', mode='a', header=False)
                # print(df)
                ...
                if buyCross:
                    ...
                    self.strategy.pos[self.bar.vtSymbol] += order.totalVolume
                else:
    ...
    # ----------------------------------------------------------------------
    def calculateBacktestingResult(self, annualDays=365):
        """
        计算回测结果
        """
        ...
        # 计算夏普比率
        # SR = 日回报率的均值/日回报率的标准差,ASR=np.sqrt(365)*SR
        for result in resultList:
            prevBalance = endBalance  # 回测时的起始本金
            endBalance += result.pnl  # 净盈亏
            returnList.append(endBalance / prevBalance - 1)
            dailyReturn = np.mean(returnList) * 100  # 每日超额回报的均值

            returnStd = np.std(returnList) * 100  # 每日超额回报的标准差
            if returnStd:
                dailySR = dailyReturn / returnStd  # 日化夏普比率

                sharpeRatio = dailySR * np.sqrt(annualDays)  # 年化夏普比率
            else:
                sharpeRatio = 0
            sharpeRatioList.append(sharpeRatio)
        # 计算盈亏相关数据
        ...
        d['sharpeRatioList'] = sharpeRatioList
    ...
    def showBacktestingResult(self):
        ...
        # result = self.calculateDailyStatistics()
        # 输出
        self.output('-' * 30)
        ...
        self.output(u'Sharpe Ratio：\t%s' % formatNumber(d['sharpeRatio']))
        ...
        psharpeRatio = plt.subplot(5, 1, 2)
        psharpeRatio.set_ylabel("sharpeRatio")
        psharpeRatio.plot(d['sharpeRatioList'], color='r', lw=0.8)
    ...
    # ----------------------------------------------------------------------
    def calculateDailyStatistics(self, annualDays=365):
        """计算按日统计的结果"""
    ...
    def showDailyResult(self, d=None, result=None):
        ...
        psharpeRatio = plt.subplot(5, 1, 2)
        psharpeRatio.set_title('sharpeRatio')
        plt.plot(d['sharpeRatio'])
...
class TradingResult(object):
    ...
    # ----------------------------------------------------------------------
    def __init__(self, entryPrice, entryDt, exitPrice, exitDt, volume, rate, slippage, size):
        """Constructor"""
        ...
        # value = [self.entryPrice, self.exitPrice, self.entryDt, self.exitDt, self.volume, self.turnover,
        #          self.commission, self.slippage, self.pnl]
        #
        # df = pd.DataFrame([value],
        #                   columns=['entryPrice', 'exitPrice', 'entryDt', 'exitDt', 'volume', 'turnover', 'commission',
        #                            'slippage', 'pnl'])
        # df.to_csv('F:\Work\Code\ TradingResult_2.csv', mode='a', header=False)
        # print(df)
