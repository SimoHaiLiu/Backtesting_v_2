...
from scipy import stats,log,exp,sqrt,pi
from trader.language import constant
from vtCalculateGreek import calculateGreeks
from trader.vtConstant import (EMPTY_FLOAT, EMPTY_INT, EMPTY_STRING, EMPTY_UNICODE)
...
########################################################################
class VtPositionData(VtBaseData,calculateGreeks):
    """持仓数据类"""
    ...
    def createFromGateway(gateway,                      # type: VtGateway
        ...
        vtPosition.delta = Greeks.calculateDelta()
        vtPosition.theta = Greeks.calculateTheta()
        vtPosition.gamma = Greeks.calculateGamma()
        vtPosition.rho = Greeks.calculateRho()
        vtPosition.vega = Greeks.calculateVega()
        return vtPosition

    # def delta(self,S, K, T, r, q, sigma, option = 'call'):
    ...
    #     return delta
    #
    # def theta(self,S, K, T, r, q, sigma, option = 'call'):
    #     d1 = (log(S / K) + (r - q + 0.5 * sigma ** 2) * T) / (sigma * sqrt(T))
    #     d2 = (log(S / K) + (r - q - 0.5 * sigma ** 2) * T) / (sigma * sqrt(T))
    #
    #     if option == 'call':
    #         theta = -S * exp(-q * T - 0.5 * (d1 ** 2)) * sigma / (2 * sqrt(2.0 * pi * T)) - r * K * exp(
    #             -r * T) * stats.norm.cdf(d2, 0.0, 1.0) + q * S * exp(-q * T) * stats.norm.cdf(d1, 0.0, 1.0)
    #     if option == 'put':
    #         theta = -S * exp(-q * T - 0.5 * (d1 ** 2)) * sigma / (2 * sqrt(2.0 * pi * T)) + r * K * exp(
    #             -r * T) * stats.norm.cdf(-d2, 0.0, 1.0) - q * S * exp(-q * T) * stats.norm.cdf(-d1, 0.0, 1.0)
    #
    ...
    #     d1 = (log(S / K) + (r - q + 0.5 * sigma ** 2) * T) / (sigma * sqrt(T))
    #
    #     gamma = exp(-q * T - 0.5 * (d1 ** 2)) / (sqrt(2.0 * pi) * S * sigma * sqrt(T))
    #
    #     return gamma
    #
    ...
    #     return (vega / 100.0)
    #
    # def rho(self,S, K, T, r, q, sigma, option = 'call'):
    #     d2 = (log(S / K) + (r - q - 0.5 * sigma ** 2) * T) / (sigma * sqrt(T))
    #
    #     if option == 'call':
    #         rho = K * T * exp(-r * T) * stats.norm.cdf(d2, 0.0, 1.0)
    #     if option == 'put':
    #         rho = -K * T * exp(-r * T) * stats.norm.cdf(-d2, 0.0, 1.0)
    #
    #     return (rho / 100.0)
