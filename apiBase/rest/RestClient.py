...
class Request(object):
    ...
    #----------------------------------------------------------------------
    def __init__(self, method, path, params, data, json, headers, callback, sign=True):
...
class RestClient(object):
    ...
    def addRequest(self,
        ...
        :return: Request
        """
        print 'RestClient, addReq',datetime.now().strftime('%H:%M:%S.%f')[:-3]   
        request = Request(method, path, params, data, json, headers, callback)
        request.extra = extra
        request.sign = sign
    ...
    def _processRequest(self, request, session):  # type: (Request, requests.Session)->None
        ...
        try:
            ...
            print 'RestClient, procReq',datetime.now().strftime('%H:%M:%S.%f')[:-3]   
            
            if request.data:
                response = session.request(request.method, url, headers=request.headers, data=request.data)
            elif request.params:
                response = session.request(request.method, url, headers=request.headers, params=request.params)                
            elif request.json:
                response = session.request(request.method, url, headers=request.headers, json=request.json)
            else:
                response = session.request(request.method, url, headers=request.headers)
                
            request.response = response
    
            httpStatusCode = response.status_code
            success = httpStatusCode / 100 == 2
            if success:                               # 2xx都算成功，尽管交易所都用200
